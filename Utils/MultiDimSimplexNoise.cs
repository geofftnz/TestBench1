...
namespace Utils
{
    ...
    /// 
    /// C# conversion by Geoff Thornburrow (geofftnz@gmail.com)
    /// </summary>
    ...
    {  // Simplex noise in 2D, 3D and 4D
        ...
        private static float F2 = 0.5f * ((float)Math.Sqrt(3.0) - 1.0f);
        private static float G2 = (3.0f - (float)Math.Sqrt(3.0)) / 6.0f;
        private static float F3 = 1.0f / 3.0f;
        private static float G3 = 1.0f / 6.0f;
        private static float F4 = ((float)Math.Sqrt(5.0) - 1.0f) / 4.0f;
        private static float G4 = (5.0f - (float)Math.Sqrt(5.0)) / 20.0f;
        ...
        public static float wrapnoise(float s, float t, float xs, float ys, float rx1, float ry1, float scale)
        {
            //float s = x / xs;
            //float t = y / ys;
            float PI2 = (float)(2.0 * Math.PI);
            float nx = rx1 + (float)(Math.Cos(s * 2.0 * Math.PI) * xs * scale) / PI2;
            float ny = ry1 + (float)(Math.Cos(t * 2.0 * Math.PI) * ys * scale) / PI2;
            float nz = rx1 + (float)(Math.Sin(s * 2.0 * Math.PI) * xs * scale) / PI2;
            float nw = ry1 + (float)(Math.Sin(t * 2.0 * Math.PI) * ys * scale) / PI2;
